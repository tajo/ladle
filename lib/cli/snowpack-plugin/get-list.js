 ...
 */
const getList = async (entries, appSrcDir) => {
      ...
      ExportNamedDeclaration: (astPath) => {
        ...
        const ast = lazyImport({
          source: t.stringLiteral(
            path
              .join(path.relative(appSrcDir, process.cwd()), entry)
              .replace(/\.tsx?$/, ".js")
          ),
...
const getProviderImport = () => {
  ...
  const providerExistsJs = fs.existsSync(providerPathJs);
  ...
  return `export const Provider = ({children}) => /*#__PURE__*/React.createElement(React.Fragment, null, children);\n`;
};
 ...
 * @param {string} appSrcDir
 */
const getListWithHmr = async (entries, appSrcDir) => {
  ...
  const [listAst] = await getList(entries, appSrcDir);
  return `${listAst}\n${getConfigImport()}\n${getProviderImport()}\n${hot}`;
};
